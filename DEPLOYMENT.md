...
- [ ] `FRONTEND_URL` is set to `https://inexledger.com`
...
## Pending Integrations (Not Yet Deployed)

| Feature | Status | Blocks |
|---------|--------|--------|
| Stripe payment integration | Pending | Billing, tier enforcement |
| Real billing/cancel in settings | Pending | Stripe |
| Server-side tier enforcement | Pending | Stripe |
| PDF export worker | Pending | Redacted export pipeline |
| MFA implementation | Pending | Security hardening |

> These items must be completed before enabling paid tiers in production.
