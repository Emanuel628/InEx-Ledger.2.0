...
const defaultCategoriesUS = [
  ...
  { name: "Phone & Internet", kind: "expense", color: "slate", tax_map_us: "utilities" },
  ...
  { name: "Supplies", kind: "expense", color: "blue", tax_map_us: "supplies" },
...
const defaultCategoriesCA = [
  ...
  { name: "Software & Subscriptions", kind: "expense", color: "blue", tax_map_ca: "office_expense" },
...
async function seedDefaultsForBusiness(db = pool, businessId) {
  ...
  const trialEligibilityResult = await targetDb.query(
  ...
  const trialEligible = trialEligibilityResult.rows[0]?.trial_eligible !== false;
  await targetDb.query(
    ...
    trialEligible
      ? [crypto.randomUUID(), businessId, "v1", "trialing", new Date().toISOString(), trialEndsAt.toISOString()]
      : [crypto.randomUUID(), businessId, "free", "free", null, null]
  );
}
