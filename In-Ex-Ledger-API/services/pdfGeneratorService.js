...
function buildCpaActionLines(reviewInsights, currency) {
  const detailLines = [
    ...
    `${reviewInsights.missingDescriptionCount} transaction${reviewInsights.missingDescriptionCount !== 1 ? "s" : ""} still need a clear description or memo.`,
    `${reviewInsights.duplicateCount} transaction${reviewInsights.duplicateCount !== 1 ? "s" : ""} are flagged as potential duplicates.`,
...
function buildChecklistItems(reviewInsights, currency, reviewDecisionSummary = {}, packageAttribution = {}, isSecure = true) {
  ...
  return [
    ...
    { badge: reviewInsights.missingDescriptionCount > 0 ? "ACTION" : "OK", title: "Descriptions", description: `${reviewInsights.missingDescriptionCount} transactions still need a usable description or memo before final handoff.` },
    { badge: reviewInsights.duplicateCount > 0 ? "ACTION" : "OK", title: "Duplicate review", description: `${reviewInsights.duplicateCount} transactions are flagged as potential duplicates and should be resolved before export is finalized.` },
