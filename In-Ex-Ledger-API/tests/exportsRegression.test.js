...
test("redacted PDF keeps attribution anonymous and checklist stays on-page", () => {
  ...
  assert.match(pdf, /Finalized by Finalizer on[\s\S]{0,80}file on 2026-05-25/i);
  assert.doesNotMatch(pdf, /emanu62893@gmail\.com/i);
  assert.doesNotMatch(pdf, /reviewer@example\.com/i);
  assert.doesNotMatch(pdf, /1 0 0 1 [\d.]+ -\d+\.\d+ Tm/);
});

test("support page distinguishes expense receipt coverage from mapped receipt support counts", () => {
  const pdf = buildPdfExport(buildFixtureOptions()).toString("latin1");
