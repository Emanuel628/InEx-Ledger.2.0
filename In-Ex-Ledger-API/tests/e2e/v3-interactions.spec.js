...
test.describe('v3 app interactions', () => {
  test('transaction create edit delete and undo flow works through v3 UI', async ({ page }) => {
    ...
    const categorySelect = transactionDrawer.locator('label').filter({ hasText: 'Category' }).locator('select')
    const accountSelect = transactionDrawer.locator('label').filter({ hasText: 'Account' }).locator('select')
    await expect(categorySelect.locator('option').nth(1)).not.toHaveText('Manage categories...')
    await categorySelect.selectOption({ index: 1 })
    await expect(accountSelect.locator('option').filter({ hasText: accountName })).toHaveCount(1)
