...
router.get("/:assetId", asyncRoute(async (req, res) => {
  const businessId = await resolveBusinessIdForUser(req.user);
  const asset = await getCapitalAsset(req.params.assetId, businessId);
  if (!asset) {
    throw new ApiError(404, "Capital asset not found.");
  }
  res.json(asset);
}));
...
router.post("/", asyncRoute(async (req, res) => {
  ...
  if (!purchase_date) {
    throw new ApiError(400, "purchase_date is required.");
  }
  if (!original_cost || Number(original_cost) <= 0) {
    throw new ApiError(400, "original_cost must be a positive number.");
  }
  if (!asset_category || !VALID_ASSET_CATEGORIES.has(asset_category)) {
    throw new ApiError(400, `asset_category must be one of: ${[...VALID_ASSET_CATEGORIES].join(", ")}.`);
  }
  if (cca_class && !VALID_CCA_CLASSES.has(cca_class)) {
    throw new ApiError(400, `cca_class must be one of: ${[...VALID_CCA_CLASSES].join(", ")}.`);
  }
  if (macrs_class && !VALID_MACRS_CLASSES.has(macrs_class)) {
    throw new ApiError(400, `macrs_class must be one of: ${[...VALID_MACRS_CLASSES].join(", ")}.`);
  }
...
router.put("/:assetId", asyncRoute(async (req, res) => {
  const body = req.body ?? {};
...
router.post("/:assetId/dispose", asyncRoute(async (req, res) => {
  const { disposed_date } = req.body ?? {};
  if (!disposed_date) {
    throw new ApiError(400, "disposed_date is required.");
  }
  const businessId = await resolveBusinessIdForUser(req.user);
