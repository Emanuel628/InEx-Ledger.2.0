...
function syncTransactionScopeUi() {
  ...
  if (subtitle) {
    subtitle.textContent = isAllScope
      ...
      : txT("transactions_scope_active_subtitle", "Active business ledger · this period");
  }
...
function renderTotals(filteredTransactions = getFilteredTransactions()) {
  ...
  if (!isAllScope && taxLabel && setAsideLabel) {
    // CA income tax (T2125) is assessed on net profit, same as US Schedule C.
    const setAsideBase = Math.max(0, totals.income - totals.expenses);
