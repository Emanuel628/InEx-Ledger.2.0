...
Working list of agreed UX changes from the 2026-06-15 review session. Statuses
below reflect implementation as of 2026-06-15.

## Entry funnel / auth

1. **Done - Email verification -> email-link only (no blocking page at signup).**
...
2. **Done - Remove `readonly` from the login fields.**
...
3. **Done - Collect country earlier - at register, not deferred to onboarding.**
   - Areas: `public/html/register.html`, `routes/auth.routes.js` register handler.

4. **Done - Move the cookie-consent banner to the bottom of the page.**
...
5. **Done - Auto-seed all default categories at business creation; remove the "Add default categories" button.**
...
6. **Done - Add a mandatory "add your first account" onboarding step; stop auto-seeding default accounts.**
...
7. **Done - Two-column form fields touch in "Desktop Version" (1280) view.**
...
8. **Done - `GET /api/review/queue` returns HTTP 500 for a fresh account** (no data).
...
9. **Done - "Add another business" silently redirects to `/subscription`.**
...
10. **Done - "Restart tour" (Reset guided setup) leaves Settings to `/transactions`**
...
11. **Done - Cookie-consent banner overlaps the Settings left-nav** at the top (same
    banner as funnel item #4 - confirmed it also covers the section list here).

12. **Done - MFA is off by default** with a stronger enrollment prompt.
...
13. **Done - Redundant Overview vs. inline sections.** The page shows Overview state
...
14. **Done - Verify empty "Update password" validation** - the button now blocks
