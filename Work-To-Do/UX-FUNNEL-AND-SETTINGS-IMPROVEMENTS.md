...
(deferred). Grouped by area. Code locations included for whoever implements.
...
2. **Remove `readonly` from the login fields.**
   ...
   which breaks password managers and is friction even when testing.
...
7. **Two-column form fields touch in "Desktop Version" (1280) view.**
   ...
   real gaps render too small. Investigated; `min-width:0` alone did not resolve.
   See PR #256 for the partial attempt.
...
13. **Redundant Overview vs. inline sections.** The page shows Overview state
    + "Manage" cards *and* renders every section inline in one long scroll, so
